...
declared the best cutomer. Write a query that retuns the person who has spent the 
most money. */
...
Q1; Find how much amount spent by each customer on best selling artists? Write a 
    query to return customer name, artist name and tatal spent?      */  
...
/* Q2; We want to find out the most popular music Genre for each country. We determine the most
	   ...
	   each country along with the top Genre. For countries where tha maximum number of purchase is
...
/* Q3;  Write a query that determines the customer that has spent the most on music for each
        country. Write a query returns the country along with the top customer and how much they
		spent. For countries where the top amount spend is shared, provide all customer who 
		spent this amount.*/ 
